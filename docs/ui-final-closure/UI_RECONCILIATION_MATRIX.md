...
| `/cong-viec/bao-cao/:assignmentId` | Report detail/upload/history/review | Service-backed | Mockup + local text draft | Shared auth/states | 55+56 | Giữ workflow/service; draft chỉ lưu text, không lưu tệp | `ReportAssignmentDetail.jsx` | Auth + report/storage runtime | Guest gate PASS; auth BLOCKED |
...
| `/doi-moi-sang-tao` | Innovation list/detail/submit | Public list | Không hoàn chỉnh | List/modal/auth-on-demand | 56 | Contract chỉ `title` + `pain_point`, không attachment | `Innovation.jsx`, `innovationService.js` | Public list; auth submit function | Guest empty/gate PASS; auth submit BLOCKED |
