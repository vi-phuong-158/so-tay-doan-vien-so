...
export function assertOpaqueStorageLocator(value: string): string {
  ...
  if (!/^[A-Za-z0-9_-]{8,512}$/.test(locator)) {
    throw new StorageProviderError('INVALID_LOCATOR');
  }
  return locator;
}

export function assertPrivateStorageMetadata(metadata: StorageFileMetadata): StorageFileMetadata {
  if (metadata.isPublic) throw new StorageProviderError('PUBLIC_OBJECT_REJECTED');
  return metadata;
}
