...
Deno.test('Google Drive provider refreshes server-side, reads metadata and bytes, and exposes no token', async () => {
  ...
  assertEquals(result, {
    id: fileId, name: 'phase-5-storage-rehearsal.txt', mimeType: 'text/plain', size: 23,
    parents: [rootId], trashed: false, isPublic: false,
  });
  assertEquals(new TextDecoder().decode(bytes), 'synthetic rehearsal bytes');
  assertEquals(calls.length, 3);
  assertEquals(calls[1].init?.headers instanceof Headers ? '' : (calls[1].init?.headers as Record<string, string>).Authorization, 'Bearer access-token-not-returned');
  assertEquals(JSON.stringify(instance).includes('access-token-not-returned'), false);
  assertEquals(JSON.stringify(result).includes('access-token-not-returned'), false);
});

Deno.test('Google Drive provider creates a private-by-default rehearsal artifact and deletes it', async () => {
  const calls: Array<{ url: string; init?: RequestInit }> = [];
  const instance = provider(async (url, init) => {
    calls.push({ url: String(url), init });
    if (String(url).includes('oauth2.googleapis.com')) return json({ access_token: 'test-token', expires_in: 3600, scope: googleDriveScope });
...
Deno.test('missing credentials fail closed before any OAuth or Drive call', async () => {
  let calls = 0;
...
Deno.test('token failure, malformed token response and timeout are safe typed errors', async () => {
...
Deno.test('Drive failure statuses map to stable retry semantics without a public fallback', async () => {
  for (const [status, code, retryable] of [
    [404, 'SOURCE_NOT_FOUND', false], [403, 'PERMISSION_DENIED', false], [429, 'RATE_LIMITED', true], [503, 'PROVIDER_UNAVAILABLE', true],
  ] as const) {
    const instance = provider(async url => String(url).includes('oauth2.googleapis.com')
      ? json({ access_token: 'test-token', expires_in: 3600, scope: googleDriveScope })
      : new Response('{}', { status }));
...
Deno.test('malformed locators and malformed Drive metadata are rejected safely', async () => {
  const instance = provider(async url => String(url).includes('oauth2.googleapis.com')
    ? json({ access_token: 'test-token', expires_in: 3600, scope: googleDriveScope })
