...
type GooglePermission = { type?: string; role?: string; allowFileDiscovery?: boolean };
type GoogleFile = {
  id?: string;
  name?: string;
  mimeType?: string;
...
function encodePath(locator: string): string {
...
export class GoogleDriveStorageProvider implements StorageProvider {
  ...
  async getMetadata(locator: string): Promise<StorageFileMetadata> {
    const response = await this.driveFetch(
      `${DRIVE_API}/files/${encodePath(locator)}?fields=id,name,mimeType,size,parents,trashed,permissions(id,type,role,allowFileDiscovery)`,
    );
  ...
  async read(locator: string): Promise<Uint8Array> {
    const response = await this.driveFetch(`${DRIVE_API}/files/${encodePath(locator)}?alt=media`);
  ...
  async put(request: StorageWriteRequest): Promise<StorageFileMetadata> {
    ...
    const response = await this.driveFetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,mimeType,size,parents,trashed,permissions(id,type,role,allowFileDiscovery)', {
      method: 'POST',
  ...
  async delete(locator: string): Promise<void> {
    await this.driveFetch(`${DRIVE_API}/files/${encodePath(locator)}`, { method: 'DELETE' });
  }
  ...
  private async readJson<T>(response: Response): Promise<T> {
    ...
    catch { throw new StorageProviderError('MALFORMED_RESPONSE'); }
  }

  private async fetchWithTimeout(url: string, init: RequestInit): Promise<Response> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), Math.min(Math.max(this.#config.timeoutMs ?? 10_000, 1_000), 30_000));
    try {
