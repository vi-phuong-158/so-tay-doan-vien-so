...
export async function processIngestionBatch(options: {
  ...
  const result: IngestionWorkerResult = {
    claimed: jobs.length, succeeded: 0, retried: 0, failed: 0, stale: 0, rpcErrors: 0,
  };
  const execute = options.execute ?? (async () => undefined);
  for (const job of jobs) {
    try {
      // P5-R0 deliberately performs no extraction or external provider action.
      await execute(job);
      const completion = await options.adminClient.rpc('complete_ingestion_job', {
        p_job_id: job.id, p_claim_token: job.claim_token, p_result: { handler: 'NO_OP_FOUNDATION' },
      });
      ...
      const transition = await options.adminClient.rpc('fail_ingestion_job', {
        ...
        p_error_code: 'NO_OP_HANDLER_FAILED', p_error_message: 'Foundation handler failed', p_retryable: true,
      });
